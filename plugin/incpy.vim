...
def generate_package_loaders(module, path):
    ...
    class loader_t: pass
    loader = loader_t()
    loader.__file__ = loader.path = os.path.join(path, 'loader.py')
    ...
    iterable = ((os.path.splitext(filename), os.path.join(path, filename)) for filename in files)
    submodules = {name : path for (name, ext), path in iterable}
    pythonx_finder = loader.vim_plugin_support_finder(path, submodules)
    yield loader.vim_plugin_packager(module, [pythonx_finder], namespace)
EOF
endfunction
